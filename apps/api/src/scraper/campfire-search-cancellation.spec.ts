...
import { CampfireScraperService } from './campfire-scraper.service';

describe('CAMPFIRE search cancellation', () => {
  it('interrupts navigation and closes context before browser within two seconds', async () => {
    const events: string[] = [];
    ...
    const scraper = new CampfireScraperService();
    const controller = new AbortController();

    const search = scraper.search({ limit: 10 }, { signal: controller.signal });
