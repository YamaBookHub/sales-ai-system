...
- `OPENAI_MODEL` が `gpt-5.6-sol` または `gpt-5.6` の場合はSOL向け、その他はLUNA向けのメール指示を使用する
- 既定モデルは `gpt-5.6-luna`。両モデルとも同じJSON契約と `docs/08_MAIL.md` の正本に従う
- メール画面で選択した `gpt-5.6-luna` / `gpt-5.6-sol` は、AI整形と意味確認のリクエストに適用する
- 画面から指定できるモデルは `SELECTABLE_AI_MODELS` の許可リストに限定し、未指定時だけ `OPENAI_MODEL` へフォールバックする
- 会社名・商品名・商品特徴は人間確認前提
