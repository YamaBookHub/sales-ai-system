...
`POST /api/projects/search/campfire` は営業リストを絞り込むAPIではなく、営業リストへ取り込む前のCAMPFIRE候補URL検索に使う。キーワード、検索用カテゴリvalue、現在公開中、終了間近、支援額レンジ、サポーター数レンジ、過去プロジェクト数レンジから変換した下限/上限、検索結果件数を受け取り、候補のタイトル、URL、支援額、サポーター数、残り日数、過去プロジェクト数を返す。検索結果件数は10件、50件、100件から選ぶ。過去プロジェクト数レンジが指定された場合は候補ページを追加確認するため、通常検索より時間がかかる場合がある。
