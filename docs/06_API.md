...
`POST /api/mails/{id}/reject` は `draft`、`in_review`、`approved` のメールを `rejected` にする。棄却理由は保存する。
...
## 8. DTO制約
CreateLeadDtoはcompanyId必須。CreateMailDraftDtoはleadId、templateKey必須。UpdateMailDtoはsubject/bodyを任意更新できる。Approveは将来manager以上に制限する。
