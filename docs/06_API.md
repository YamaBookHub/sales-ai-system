...
検索APIの取得上限は左側の検索欄で「何件取りに行くか」を決めるために使う。左側で過去プロジェクト数を指定した場合は、取得上限より多めに候補を確認してから該当候補を返す。画面の初期状態では過去プロジェクト数を指定せず、高速検索を優先する。最大表示件数は検索条件ではなく、画面右側の「CAMPFIRE検索結果」で取得済み候補の表示数を切り替えるために使う。
