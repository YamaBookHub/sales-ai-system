...
`POST /api/projects/search/campfire` は営業リストを絞り込むAPIではなく、営業リストへ取り込む前のCAMPFIRE候補URL検索に使う。キーワード、検索用カテゴリvalue、カテゴリ表示名、取得上限、過去プロジェクト数の下限/上限を受け取り、候補のタイトル、URL、支援額、サポーター数、残り日数、過去プロジェクト数を返す。画面では検索時点で支援額、サポーター数、公開状態による絞り込みを行わない。

カテゴリが指定された場合は、CAMPFIREカテゴリページを開くだけでなく、取得した案件ページ側のカテゴリ名も確認し、選択カテゴリと一致しない候補を除外する。

検索APIの取得上限は左側の検索欄で「何件取りに行くか」を決めるために使う。左側で過去プロジェクト数を指定した場合は、取得上限より多めに候補を確認してから該当候補を返す。画面の初期状態では100件以上の実行者を除外するため、過去プロジェクト数の上限99を送る。最大表示件数は検索条件ではなく、画面右側の「CAMPFIRE検索結果」で取得済み候補の表示数を切り替えるために使う。
...
過去プロジェクト数は左側の検索条件、検索結果エリアの表示条件、URL取り込み後の案件詳細で確認する。
