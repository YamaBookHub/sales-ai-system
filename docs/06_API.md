...
`GET /api/projects/categories/campfire` はCAMPFIREの検索ページからカテゴリ候補を取得し、画面の検索プルダウンに使う。カテゴリは表示名とCAMPFIREカテゴリURLを返す。
`POST /api/projects/search/campfire` は営業リストを絞り込むAPIではなく、営業リストへ取り込む前のCAMPFIRE候補URL検索に使う。キーワード、カテゴリURL、現在公開中、終了間近、支援額の下限/上限、サポーター数の下限/上限を受け取り、候補のタイトル、URL、支援額、サポーター数、残り日数を返す。
