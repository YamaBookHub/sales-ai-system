...
- 画面からのテンプレート取り込み・編集フォーム
- AI生成時に `templateKey` からDBテンプレートを参照すること
...
次に着手する作業は、テンプレート管理画面を追加するか、AI生成時に `templateKey` からDBテンプレートを参照すること。資料閲覧を画面に表示する作業も次候補。
