...
8. 会社block、連絡先配信停止、正規化送信先の重複は、下書きから送信記録まで共通guardで拒否する。
...
実送信providerを安全に差し替える土台と、Gmail OAuth providerを追加済み。
...
- `infrastructure/gmail-mail.sender.ts`: Gmail OAuth送信provider。メール以外のチャネルは拒否
- `PrismaMailWorkflowRepository`: `queued` のみを `sending` にclaimし、`sent` / `failed` へ遷移
- providerの`validate`をclaim前に実行し、未設定providerや非対応チャネルを送信中・失敗状態へ進めない
- `sending` で停止したメールは自動再送せず、外部provider確認後の `mark-sent` で手動復旧する。イベントpayloadに `recoveredFrom: sending` を残す
...
- Mail workspaceの「追跡リンクを本文へ追加」: 会社資料URLから `company_material` リンクを作り、保存前の本文へ差し込む
- 下書き保存時の資料URL自動追跡: 「会社資料」「資料」などの行、またはPDF/OfficeファイルURLだけを対象にする
- `AnalyzeLeadUseCase`: 資料閲覧回数・最終閲覧・アポ角度をAI分析の入力、営業角度、次アクション、根拠、注意点へ反映
...
- `GenerateMailDraftUseCase`: 有効な定型文を `templateKey` から参照して本文・件名へ適用
...
- `sendMethod` に応じて `email` / `site_message` / `contact_form` 定型文を生成画面へ推奨表示

残っている実装:

- `site_message` / `contact_form` の外部サイト実送信provider連携
- `MailSender` へ `sendMethod` / 対象URLを渡し、providerがメール以外を拒否する安全境界は実装済み
- Mail workspaceはサイトDM / 問い合わせフォーム案件で「手動送信済みにする」と表示し、メール送信との取り違えを防ぐ
